...
def _infer_vendor_from_name(hostname: str) -> str:
    ...
    if "bdcom.gp3600" in token or "bdcom.gpon" in token:
        return "bdcom_gpon"
    if model.startswith("fd160"):
        return "fd160"
    if model.startswith("gr-ep-olt1-4"):
        return "gr_ep_olt1"
    if model.startswith("gr-ep-olt2-8-2ac"):
        return "gr_ep_olt2"
